...
Router.route('/dataSync', {
    waitOn: function () {
        return [this.subscribe('dataStationHourlyReSyncRecord')]
    }
});
Router.route('/dataImportExport', {
    waitOn: function () {
        return [this.subscribe('station'), this.subscribe('city')]
    }
});
...
Router.route('/airQualityReview', {
    waitOn: function () {
        return [this.subscribe('airQuality'), this.subscribe('limit')]
    }
});
Router.route('/airQualityPublish', {
    waitOn: function () {
        return [this.subscribe('county'), this.subscribe('airQuality'), this.subscribe('dataAirQuality'), this.subscribe('limit')]
    }
});
...
Router.onBeforeAction(function (req, res, next) {
    ...
    //    }
    //})
    else if (!Meteor.userId() || this.url == '/login') {
        this.layout("emptyLayout");
        this.render('login');
    } else {
        this.layout("layout");
